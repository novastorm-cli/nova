...
export async function startCommand(options: StartOptions = {}): Promise<void> {
  ...
  sp[license.valid ? 'succeed' : 'warn'](license.valid ? `License OK (${license.tier}, ${license.devCount} dev(s)).` : chalk.yellow(`License warning: ${license.message ?? 'Invalid license.'}`));
  ...
  const stackLabel = [stack.framework, ...(stack.additionalStacks ?? [])].filter(s => s !== 'unknown').join(' + ') || 'unknown';
  sp.succeed(`Detecting project... ${chalk.cyan(stackLabel)} (${chalk.dim(stack.typescript ? 'TypeScript' : stack.language || 'unknown')})`);

  // ── 5. Scaffold if needed ────────────────────────────────────────
  const scaffResult = await runScaffold(cwd, config, options, devCommand, devPort);
  if (scaffResult) { devCommand = scaffResult.devCommand; devPort = scaffResult.devPort; }
  ...
  const projectMap = await indexer.index(cwd, { frontend: config.project.frontend, backends: config.project.backends });
  ...
  try {
    ...
    try { const r = await fetch('http://127.0.0.1:11434/api/tags'); if (r.ok) { embProvider = 'ollama'; embUrl = 'http://127.0.0.1:11434'; } } catch {}
    if (embProvider === 'tfidf') { const k = config.apiKeys.provider === 'openai' ? config.apiKeys.key : process.env.OPENAI_API_KEY; if (k) { embProvider = 'openai'; embKey = k; } }
    const embSvc = createEmbeddingService({ provider: embProvider, apiKey: embKey, baseUrl: embUrl });
    const vs = new VectorStore();
    ragIndexer = new RagIndexer(embSvc, vs);
    const pLabel = embProvider === 'openai' ? 'OpenAI' : embProvider === 'ollama' ? 'Ollama' : 'TF-IDF';
  ...
  } catch (err) { sp.warn(`RAG indexing skipped: ${err instanceof Error ? err.message : String(err)}`); }
  ...
  try { await devServer.spawn(devCommand, cwd, devPort); } catch (err) {
  ...
  if (actualPort && actualPort !== devPort) { sp.succeed(`Dev server started on port ${chalk.yellow(actualPort)}`); devPort = actualPort; proxyPort = devPort + PROXY_PORT_OFFSET; }
  else { sp.succeed('Dev server started'); }
  ...
  setTimeout(() => wsServer.sendEvent({ type: 'analysis_complete', data: { fileCount: analysis.fileCount, methodCount: analysis.methods.length } }), 2000);

  // ── 13. Browser ──────────────────────────────────────────────────
  await openBrowser(`http://localhost:${proxyPort}`, { noOpen: options.noOpen });

  // ── 14. Git ──────────────────────────────────────────────────────
  try { (await import('node:child_process')).execSync('git rev-parse --git-dir', { cwd, stdio: 'ignore' }); } catch {
  ...
  const commitQueue = new CommitQueue(gitManager, { allowProtectedBranchCommits: config.git?.allowProtectedBranchCommits });
  if (llmClient) {
    ...
    executorPool = new ExecutorPool(
      new Lane1Executor(cwd, pathGuard), new Lane2Executor(cwd, llmClient, gitManager, pathGuard, commitQueue),
      eventBus, llmClient, gitManager, cwd, config.models.micro, config.models.standard, config.models.strong,
      agentPromptLoader, pathGuard, undefined, commitQueue,
    );
  }

  let autoFixer: any = null;
  if (llmClient && executorPool) autoFixer = new ErrorAutoFixer(cwd, llmClient, gitManager, eventBus, wsServer, projectMap, commitQueue, config.models.micro);
  ...
  wsServer.onSecretsSubmit((secrets: Record<string, string>) => {
    ...
    wsServer.sendEvent({ type: 'status', data: { message: `Saved ${Object.keys(secrets).length} secret(s)` } });
  });
  ...
  setTimeout(async () => {
    ...
    if (isNonInteractive(options) || config.behavior.confirmTasks === false) {
      eventBus.emit({ type: 'observation', data: { screenshot: Buffer.alloc(0), transcript: fixTask.description, currentUrl: `file://${cwd}`, timestamp: Date.now(), autoExecute: true } as any });
    } else {
      wsServer.sendEvent({ type: 'pending_tasks', data: { tasks: [{ id: fixTask.id, description: 'Fix startup build errors', lane: 3 }], message: `Press Y to execute — ${fixTask.description.slice(0, 200)}` } });
    }
  }, 4000);
  ...
  devServer.onError((e: string) => { if (!shuttingDown) wsServer.sendEvent({ type: 'status', data: { message: `Dev server error: ${e}` } }); });

  const chat = new NovaChat();
  chat.onCommand(async (cmd) => {
    switch (cmd.type) {
      case 'text':
        if (!brain) { chat.log(chalk.yellow('AI not configured. Use /settings to add a key.')); return; }
        lastObservation.current = { screenshot: Buffer.alloc(0), transcript: cmd.args, currentUrl: `file://${cwd}`, timestamp: Date.now() };
        eventBus.emit({ type: 'observation', data: lastObservation.current });
        break;
      case 'confirm':
        if (pendingTasks.length === 0) { chat.log(chalk.dim('Nothing to confirm.')); return; }
        eventBus.emit({ type: 'observation', data: { screenshot: Buffer.alloc(0), transcript: pendingTasks.map(t => t.description).join('; '), currentUrl: `file://${cwd}`, timestamp: Date.now(), autoExecute: true } as any });
        pendingTasks.length = 0;
        break;
      case 'cancel': pendingTasks.length = 0; chat.log(chalk.yellow('Tasks cancelled.')); break;
      case 'settings': chat.log(await handleSettingsCommand(cmd.args, config, configReader, cwd)); break;
      case 'help': chat.log([chalk.bold('\nNova Commands\n'), `  ${chalk.cyan('text')}    Send as code change request`, `  ${chalk.cyan('/settings')} View/change settings`, `  ${chalk.cyan('/help')}    Show this help`, `  ${chalk.cyan('y/n')}     Confirm/cancel pending`, ''].join('\n')); break;
      case 'status': chat.log([chalk.bold('\nStatus\n'), `  Project: ${cwd}`, `  Stack: ${projectMap.stack.framework}`, `  Dev: localhost:${devPort}`, `  Proxy: localhost:${proxyPort}`, `  Pending: ${pendingTasks.length}`, ''].join('\n')); break;
      case 'map': chat.log(chalk.cyan(`Opening project map: http://localhost:${proxyPort}/nova-project-map`)); break;
    }
  });
  chat.start();
  earlyExit = false;
  ...
  let fc = 0; process.on('SIGINT', () => { if (++fc >= 2) process.exit(1); });

  if (isNonInteractive(options) && !process.stdin.isTTY) {
    if (process.stdin.readableEnded) { await new Promise(r => setTimeout(r, 500)); await shutdown(); }
    else { process.stdin.resume(); await Promise.race([new Promise<void>(r => { process.stdin.on('close', r); process.stdin.on('end', r); }), new Promise(r => setTimeout(r, 30_000))]); await shutdown(); }
  } else { await new Promise(() => {}); }
}

// ── Local helpers (kept in this file to stay ≤400 lines) ──────────────

async function acquirePorts(devPort: number, proxyPort: number, options: StartOptions): Promise<{ devPort: number; proxyPort: number }> {
  ...
  if (!dBusy && !pBusy) { sp.succeed('Ports available'); return { devPort, proxyPort }; }
...
async function recoverDevServer(err: unknown, devCommand: string, cwd: string, devPort: number, options: StartOptions, llmClient: any, devServer: DevServerRunner): Promise<void> {
  ...
  if (/EADDRINUSE|address already in use/i.test(msg)) { choices.push({ name: `[k] Kill port ${devPort} & retry`, value: 'kill-retry' }, { name: '[p] Change port', value: 'change-port' }); }
  if (/Cannot find module|MODULE_NOT_FOUND/i.test(msg)) choices.push({ name: 'Run npm install & retry', value: 'install-retry' });
  if (/EJSONPARSE|JSON/.test(msg)) choices.push({ name: 'Fix package.json & retry', value: 'fix-json-retry' });
  if (llmClient) choices.push({ name: 'AI fix', value: 'ai-fix' });
  choices.push({ name: 'Exit', value: 'exit' });

  while (true) {
    const action = await select({ message: 'What would you like to do?', choices }).catch(() => 'exit');
    if (action === 'exit') process.exit(0);
    try {
      if (action === 'kill-retry') { await PortManager.killPort(devPort); await devServer.spawn(devCommand, cwd, devPort); return; }
      if (action === 'change-port') { const np = await input({ message: 'Port:', default: String(devPort + 10) }); await devServer.spawn(devCommand, cwd, parseInt(np, 10)); return; }
      if (action === 'install-retry') { (await import('node:child_process')).execSync('npm install', { cwd, stdio: 'inherit' }); await devServer.spawn(devCommand, cwd, devPort); return; }
      if (action === 'fix-json-retry') {
        const { readFileSync, writeFileSync } = await import('node:fs');
        const p = path.join(cwd, 'package.json');
        let c = readFileSync(p, 'utf-8').replace(/,(\s*[}\]])/g, '$1');
        writeFileSync(p, c);
        if (llmClient) { try { JSON.parse(c); } catch { const resp = await llmClient.chat([{ role: 'system', content: 'Fix this JSON. Output ONLY valid JSON.' }, { role: 'user', content: c }], { temperature: 0, maxTokens: 4096 }); let f = resp.content.trim(); const m = f.match(/```(?:json)?\n([\s\S]*?)```/); if (m) f = m[1].trim(); JSON.parse(f); writeFileSync(p, f); } }
      ...
      if (action === 'ai-fix') {
        ...
        const resp = await llmClient.chat([{ role: 'system', content: 'Output fixed files:\n=== FILE: path ===\ncontent\n=== END FILE ===' }, { role: 'user', content: `Error: ${msg.slice(0, 800)}\nUser: ${desc.trim()}` }], { temperature: 0, maxTokens: 4096 });
        const { mkdirSync, writeFileSync: wf } = await import('node:fs');
        let m: RegExpExecArray | null; const re = /=== FILE: (.+?) ===\n([\s\S]*?)\n=== END FILE ===/g;
        while ((m = re.exec(resp.content)) !== null) { mkdirSync(path.dirname(path.join(cwd, m[1].trim())), { recursive: true }); wf(path.join(cwd, m[1].trim()), m[2]); }
        await devServer.spawn(devCommand, cwd, devPort);
        return;
      }
